...
class Loader(Dataset):
    ...
    def __init__(self, VOC2012_dir, flag, resize, transforms):
        ...
        if self.flag == 'train':
            # self.imgnames = self.lines[:10] # Tip : you can adjust the number of images and run the quickly during debugging.
            self.imgnames = self.lines[self.fold:]

        else:
            # self.imgnames = self.lines[10:15] # Tip : you can adjust the number of images and run the quickly during debugging.
            self.imgnames = self.lines[:self.fold]
