...
int main() {
	...
	Mat input1 = imread("input2.jpg", CV_LOAD_IMAGE_COLOR); // right image
	Mat input2 = imread("input1.jpg", CV_LOAD_IMAGE_COLOR); // left image
	...
	int* templ_x = new int[k];
	int* templ_y = new int[k];
	int* tempr_x = new int[k];
	int* tempr_y = new int[k];
	int* bestPL_x = new int[k];
	int* bestPL_y = new int[k];
	int* bestPR_x = new int[k];
	int* bestPR_y = new int[k];

	for (int i = 0; i < s; i++) {
		int* inliner_x = new int[k];
		int* inliner_y = new int[k];
		int count = 0;
		int maxInliner = 0;
		...
		Mat tempA12 = cal_affine<float>(templ_x, templ_y, tempr_x, tempr_y, n);
