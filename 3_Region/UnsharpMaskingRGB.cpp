...
int main() {
	...
	//output = UnsharpMask(input, 1, 1, 1, "adjustkernel", 0.5);
			...
			if (!strcmp(boundary_proc, "zero-paddle")) {
				for (int a = -N; a <= N; a++) {
					for (int b = -N; b <= N; b++) {
						...
						if ((i + a <= row - 1) && (i + a >= 0) && (j + b <= col - 1) && (j + b >= 0)) { //if the pixel is not a border pixel
							sum1_0 += kernelvalue * (float)(input.at<C>(i + a, j + b)[0]);
							sum1_1 += kernelvalue * (float)(input.at<C>(i + a, j + b)[1]);
							sum1_2 += kernelvalue * (float)(input.at<C>(i + a, j + b)[2]);
						}
					}
				}
				L.at<C>(i, j)[0] = (G)sum1_0;
				L.at<C>(i, j)[1] = (G)sum1_1;
				L.at<C>(i, j)[2] = (G)sum1_2;
			}
				...
				for (int a = -N; a <= N; a++) {
					for (int b = -N; b <= N; b++) {
						...
						sum1_0 += kernelvalue * (float)(input.at<C>(tempa, tempb)[0]);
						sum1_1 += kernelvalue * (float)(input.at<C>(tempa, tempb)[1]);
						sum1_2 += kernelvalue * (float)(input.at<C>(tempa, tempb)[2]);
					}
				}
				L.at<C>(i, j)[0] = (G)sum1_0;
				L.at<C>(i, j)[1] = (G)sum1_1;
				...
				L.at<C>(i, j)[0] = (G)(sum1_0/sum2);
				L.at<C>(i, j)[1] = (G)(sum1_1/sum2);
				L.at<C>(i, j)[2] = (G)(sum1_2/sum2);
			}
		}
	}
